...
# Converts a .s3g file to a Wavefront .obj file which can be viewed with
# Blender or a similar 3D viewer. This allows you to debug / visualize the
# position of the extruder and the flow or absence of flow of plastic
# during the print before sending the design to the printer.

# TODO: This was adapted from s3g-decompiler.py and some work needs to be
# done to refactor so less of the logic is copied and pasted.

# Globals for the OBJ generation
vertices = 0
minExtrusion = 0
faceList = []

def drawPoint(parsed):
    global vertices, minExtrusion, faceList
    #print "Absolute move to (%i,%i,%i,%i,%i) at DDA %i" % parsed
    print "v %f %f %f" % (parsed[0] * 0.0011, parsed[1] * 0.0011, parsed[2] * 0.00025)
    if parsed[3] < minExtrusion:
        faceList.append(vertices)
        minExtrusion = parsed[3]
    vertices = vertices + 1
